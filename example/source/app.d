...
void main(string[] args){
	...
	uint genType  = args.length > 5 ? args[5].to!GenType() : GenType.separateAxesRand;
	...
	alias R = Rect!(ubyte[3]);
	R[] unpackedRects, rects;
	void doPacking(){
		unpackedRects = new R[](uniform(800, 1350));
		foreach(ref rect; unpackedRects){
			if(genType == GenType.separateAxesRand){
				if(!uniform(0, 8)){
				...
				if(genType == GenType.rand && !uniform(0, 16)){
					rect.w = uniform(24, 96);
					rect.h = uniform(24, 96);
				}else{
					rect.w = uniform(10, 24);
			...
			rect.userData = [
				cast(ubyte)uniform(16, 255),
				cast(ubyte)uniform(16, 255),
				cast(ubyte)uniform(16, 255),
			];
		}
		auto packStartTime = MonoTime.currTime;
		rects = atlas.pack(unpackedRects, width, height, method, padding);
		auto packEndTime = MonoTime.currTime;
		writeln("");
		writeln("Took ", (packEndTime - packStartTime).total!"nsecs"() / cast(double)(1.msecs.total!"nsecs"()),"ms to pack with ",method);
